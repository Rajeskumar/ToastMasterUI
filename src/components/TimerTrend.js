...
class TimerTrend extends React.Component {
  constructor(props) {
    super(props);
  ...
  computeChartData(){
      var chartData =[];
      var eventIdArr = [];
      var rawData = this.props.timerReport;
      ...
      this.state = {
        chartTimerData : chartData,
      };
      console.log('ChartTimerData : ', chartData);
  }
  render() {
    return (
      <Paper>
        {this.state.chartTimerData ? <Chart
          data={this.state.chartTimerData}
        >
          <ArgumentAxis />
          <ValueAxis />
